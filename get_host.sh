...
do
       ssh -i internal.pem -q -o ConnectTimeout=3 ubuntu@"$host" "hostname"
       if [ $? != 0 ]; then
       $AWSD --profile shaadiprod --region us-east-1 ec2 describe-instances --query 'Reservations[*].Instances[*].[InstanceId]' --filter "Name=private-ip-address,Values=$host"
       fi
done 